...
hospitals_path = os.path.join(destination_path, "hospitals.csv")
primary_care_path = os.path.join(destination_path, "primary_care_facilities.csv")
urgent_care_path = os.path.join(destination_path, "urgent_care_facilities.csv")
va_path = os.path.join(destination_path, "va_facilities.csv")

primary_care_passthrough = ['name', 'address', 'city', 'state', 'zip', "phone", 'LAT', 'LON']
...
primary_care_header = primary_care_passthrough + list(primary_care_static.keys()) + primary_care_blank + ["id"]

va_passthrough = ['id', 'name', 'address', 'city', 'state', 'zip', "phone", 'ownership', 'emergency', 'quality', 'LAT', 'LON']
va_static = {"type": "VA Facility"}
va_blank = ["EBTPU","PCT","PRRP","SIPU","PTSD_DOM","DH","WSDTT","SUPT","WTRP"]
...
if __name__ == "__main__":
    ...
    with open(mii_filename, "r", encoding="utf8") as mii_f, \
        ...
        uc = csv.DictWriter(uc_f, fieldnames=mii.fieldnames) #passes through row unmodified
